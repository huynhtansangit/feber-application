...
interface MyApprovedUploadsProps {
    // User Profile
...
class MyApprovedUploads extends React.Component<MyApprovedUploadsProps, any> {

    constructor(props: MyApprovedUploadsProps) {
        super(props);
        this.state = {
                            ...
                            <div className="ms-Grid">
                                <div className="ms-Grid-row" style={{ marginTop: "-10px" }}>
                                    <div className="ms-Grid-col ms-sm12" style={{ paddingLeft: "0px" }}>
                                        <RbLabel className="header-title" size={LabelSize.Large}> My Approved Uploads </RbLabel>
                                    </div>
                                ...
                                ((this.props.isLoading === true) ?
                                    ...
                                    <div>
                                        ...
                                        {(Helper.setEmptyArrayIfNull(this.props.searchResultsObj?.results).length > 0) ?
                                            this.props.searchResultsObj?.results.map((searchResult: any) =>
    ...
    callSearch(status = "", isReloaded = false) {
        ...
        this.props.getSearchResults(
            this.props.refinements.keyword + " ContentType:ResearchReports ContentType:LessonsLearned ContentType:Thesis ContentType:Paper SubmitterOWSUSER:" + this.props.userProfile?.name
    ...
    searchNewText(text = "*") {
        ...
        let href = "/MyApprovedUploads" + window.location.href.split("/MyApprovedUploads")[1];
    ...
    handleSearch(property: string, value: any, shouldCallSearch = true) {
        Helper.runNewTask(() => {
            this.props.handleSearch(property, value, "/MyApprovedUploads",
                () => {
