...
class Statistic_Report extends React.Component<Statistic_ReportProps, any> {
    ...
    render() {
        ...
        let element = (
            <div className="ms-Grid">
                ...
                <RbRadio itemWidth={12} items={redioItems} isHorizontal={true} defaultValue={"download"}
                ...
                />
                    <div className="ms-Grid-col ms-sm6">
                        <div className="ms-Grid" style={{paddingTop: "15px"}}>
                            ...
                            <div className="ms-Grid-row common-padding-row">
                                ...
                                <div className="ms-Grid-col ms-sm6">
                                    <Dropdown
                                        ...
                                        onChange={(event, option: any) => { this.setState({ fromMonth: option.key }); }}>
                                    </Dropdown>
                                </div>
                                <div className="ms-Grid-col ms-sm3">
                                    <Dropdown
                                        ...
                                        onChange={(event, option: any) => { this.setState({ fromYear: option.key }); }}>
                                    </Dropdown>
                                </div>
                            </div>
                            ...
                            <div className="ms-Grid-row common-padding-row">
                                ...
                                <div className="ms-Grid-col ms-sm6">
                                    <Dropdown
                                        ...
                                        options={Constants.DD_MONTH_OPTIONS}
                                        onChange={(event, option: any) => { this.setState({ toMonth: option.key }); }}>
                                    </Dropdown>
                                </div>
                                <div className="ms-Grid-col ms-sm3">
                                    <Dropdown
                                        ...
                                        options={Helper.generateYearsArray()}
                                        onChange={(event, option: any) => { this.setState({ toYear: option.key }); }}>
                                    </Dropdown>
                                </div>
                            </div>
                            ...
                            {/* Buttons */}
                            <div className="ms-Grid-row common-padding-row">
                                <RbButton label="Download report" size={ButtonSize.Small} onClick={this.view} />
                            </div>
    ...
    view() {
        if ((this.state.fromMonth >= this.state.toMonth && this.state.fromYear === this.state.toYear)
            ...
            switch (this.props.exportMode) {
                ...
                case "delete":
                case "search": {
                    functionToRun = this.systemListsSrv.getAllListItems(Constants.STATISTICS[(this.props.exportMode as string).toUpperCase()], Environment.rootWeb, "ID", false, "",
                        (this.props.exportMode !== "search") ? ["*", "Submitter/Title"] : ["*", "Author/Title"],
                        (this.props.exportMode !== "search") ? ["Submitter"] : ["Author"]);
                    break;
                }
                case "upload": {
                    functionToRun = new Promise((resolve: any) => {
                        this.searchSrv.searchAllResults("FeberApproveDate=" + (this.state.fromMonth + "/1/" + this.state.fromYear + ".."
                            + this.state.toMonth + "/" + Helper.getLastDate(this.state.toYear, this.state.toMonth - 1) + "/" + this.state.toYear)
                            + " ContentType:ResearchReports ContentType:LessonsLearned ContentType:Thesis ContentType:Paper", 1, [], null, (results: any[]) => {
                                resolve(results);
                            });
                    });
                    break;
            ...
            functionToRun.then((results: any[]) => {
                let rs: any[] = results.filter((item: any) => {
                    if (this.props.exportMode === "download" || this.props.exportMode === "order"
                        || this.props.exportMode === "delete" || this.props.exportMode === "search") {
    ...
    generateChartData() {
        let results: any[] = [];
        let drawData = Helper.sortObjects(this.state.drawData, (this.props.exportMode !== "upload") ? "EventDate" : "FeberApproveDate");
        drawData.forEach((item: any) => {
            let date = new Date((this.props.exportMode !== "upload") ? item.EventDate : item.FeberApproveDate);
            let monthString = (((date.getMonth() + 1) < 10) ? ("0" + (date.getMonth() + 1)) : (date.getMonth() + 1)) + "/" + date.getFullYear();
            let division = (this.props.exportMode !== "upload") ? item.Division : item.FeberDivision;
            if (results.length === 0 || (results.length > 0 && results.filter(rs => rs.id === division).length === 0)) {
                results.push({
                    id: division,
                    data: [{
                        x: monthString,
    ...
    ensureLineChart(results: any[]) {
        ...
        // distinct
        monthList = monthList.filter(function (value, index, self) { return self.indexOf(value) === index; });
        // sort
        let sortObjs: any[] = [];
        for (let index = 0; index < monthList.length; index++) {
            ...
            sortObjs.push({ date: (new Date(year, month - 1, 1, 0, 0, 0)).toISOString(), stringMonth: monthList[index] });
        }
        sortObjs = Helper.sortObjects(sortObjs, "date");
        // fill empty data
        let minDate = new Date(this.state.fromYear, this.state.fromMonth - 1, 1, 0, 0, 0);
        let maxDate = new Date(this.state.toYear, this.state.toMonth - 1, Helper.getLastDate(this.state.toYear, this.state.toMonth - 1), 23, 59, 59);
        do {
            if (minDate < maxDate) {
                let found: any = false;
                ...
                if (found === false) {
                    ...
                    sortObjs.push({ date: (new Date(year, month, 1)).toISOString(), stringMonth: (((month + 1) < 10) ? ("0" + (month + 1)) : (month + 1)) + "/" + year });
                }
            }
    ...
    changeDivision() {
        ...
        if (chartData.length > 0) {
            if (this.state.selectedDivison === "") {
                let sumData: any[] = JSON.parse(JSON.stringify(chartData[0].data)) as any[];
                let index: number = 0;
                ...
                let filteredData = (this.state.chartData as any[]).filter(x => x.id === this.state.selectedDivison);
    ...
    downloadReport() {
        ...
        this.excelSrv.exportStatistics(this.props.exportMode, this.state.drawData).then(() => {
            this.props.showToastMessage(
...
export default connect(mapStateToProps, { showDialog, showToastMessage, updateField })(Statistic_Report);