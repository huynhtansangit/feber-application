...
// fixing
  getAllListItems(
    listName: string,
    webUrl: string = Environment.rootWeb,
    sortField: string = "ID",
    ...
    let promise: Promise<any[]> = new Promise((resolve) => {
      ...
      this._getAllListItems(
        items.top(4000).orderBy(sortField, isAscending).getPaged()
      )
        .then((results: any[]) => {
          resolve(results);
        })
  ...
  private _getAllListItems(
    itemsPromise: Promise<PagedItemCollection<any[]>>,
    finalResults: any[] = [],
    resolveCallback: any = null
  ): Promise<any[]> {
    let fResult: any[] = finalResults;
    let promise: Promise<any[]> = new Promise((resolve) => {
      itemsPromise
        .then((rs: PagedItemCollection<any[]>) => {
          fResult.push(...rs.results);
          if (rs.hasNext === true) {
            this._getAllListItems(
              rs.getNext(),
              fResult,
              resolveCallback !== null ? resolveCallback : resolve
            );
  ...
  goToSiteByFullUrl(fullSubUrl: string):void {
    this.sp.setup({
      sp: {
        headers: {
          Accept: "application/json; odata=verbose",
