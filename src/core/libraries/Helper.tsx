...
class Helper {
  ...
  static generateYearsArray(yearRange: number = 10) {
    ...
    for (
      let year = currentYear - yearRange;
  ...
  static openDialog(url: string) {
    ...
    window.open(
      ...
      "_blank",
      "width=" +
        (window.screen.availWidth / 3) * 2 +
        "px,height=" +
        (window.screen.availHeight / 3) * 2 +
        "px,centerscreen=yes,scrollbars=yes,resizable=yes"
    );
  }
  ...
  static resizeColumnByScreenWidth(input: number) {
    return (
      ...
      ((document.body.clientWidth * 7) / 12)
    ); // Screen real width - 2 X paddings
  }
  ...
  static getListName(
    ...
    if (
      departmentName !== "NoDepartmentReports" &&
      documentType !== Constants.DOCUMENT_TYPE.LL &&
      documentType !== Constants.DOCUMENT_TYPE.Paper
    ) {
      // RnD or Thesis
  ...
  static getListNameUrl(item: any) {
    let result: string;
    let deptName = item.FeberDepartment.replace("/", "_").replace("_", "_");
    switch (item.SecurityClass) {
      case "": {
        result = deptName;
      }
      case "1 Internal": {
      ...
      if (authorDisplayName.indexOf(")") > -1) {
        ...
        for (let i = 0; i < authorsArr.length; i++) {
          ...
          if (authorsArr[i].indexOf("(") >= 0) {
            ...
            if (
              result.toUpperCase().trim().indexOf(authorName.toUpperCase()) ===
              -1
            ) {
        ...
        // Old data for Thesis and Paper does not have department with brackets
        let authorsArr = authorDisplayName.split(";");
        for (let i = 0; i < authorsArr.length; i++) {
          if (
            result.toUpperCase().trim().indexOf(authorsArr[i].toUpperCase()) ===
            -1
          ) {
            result += (result !== "" ? " ; " : "") + authorsArr[i];
          }
  ...
  static generateAccessMediatorPathByGuid(guid: string) {
    return (
      Environment.spaRootPageUrl + "index.aspx?#/AccessMediator?Guid=" + guid
    );
  }
  ...
  static showTrackingLog(title:string,message: any) {
