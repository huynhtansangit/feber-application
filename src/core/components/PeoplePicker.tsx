...
export class PeoplePicker extends BaseComponent<any, IPeoplePickerExampleState> {
  ...
  constructor(props: any) {
    ...
    this.state = {
      ...
      isValid: null,
      principalType: (!_.isUndefined(this.props.principalType)) ? this._principleType[this.props.principalType] : 1,
    };
  }
  ...
  private _renderNormalPicker() {
    ...
    return (
      <div className={(this.state.isValid === false) ? "invalid" : ""}>
        <NormalPeoplePicker
          itemLimit={(!_.isNil(this.props.itemLimit)) ? this.props.itemLimit : undefined}
          ...
          inputProps={{
            ...
            'placeholder': (this.state.principalType !== this._principleType.DistributionList
              && this.state.principalType !== this._principleType.SecurityGroup)?
              'Enter names or email addresses...' : 'Enter department names...'
          }}
          ...
          onChange={(items: IPersonaProps[] | undefined) => {
            if (!_.isUndefined(items)) {
              ...
              this.setState({
                ...
                isValid: ((this.props.isRequired === true) ? (items.length > 0) : true)
              }, () => {
        ...
        />
        {(this.props.isRequired === true && this.state.isValid === false) ? <p className="ms-TextField-errorMessage" style
          ={errorMessageStyle}>
  ...
  public validate() {
    ...
    if (this.props.isRequired === true) {
      if ((!_.isUndefined(this.state.currentSelectedItems)) ? this.state.currentSelectedItems.length === 0 : true) {
        result = false;
        this.setState({
          isValid: false
        });
  ...
  public _onPasteValue(clipboardData: DataTransfer) {
    this._event = "paste";
    this._clipboardData = clipboardData.getData('Text').replace(/(?:\r\n|\r|\n)/g, "");
  }
    ...
    this.getUserSuggestions(peoplePickerQuery).then((results: any[]) => {
      ...
      results.forEach(result => {
        if (result.EntityData.PrincipalType === "SharePointGroup") {
          getSpUserFuncs.push(new Promise((resolve) => {
            resolve({
              data: {
  ...
  public getUserSuggestions(query: PeoplePickerQuery): Promise<any> {
    let promise = new Promise((resolve) => {
      axios.post(Environment.rootWeb + '/_api/contextinfo', null, {
        headers: {
          'Accept': 'application/json;odata=verbose',
          'Content-Type': 'application/json'
        }
      }).then(xRequest => {
        const digest = (xRequest as any).data.d.GetContextWebInformation.FormDigestValue;
        const headers = {
          'Accept': 'application/json;odata=verbose',
        ...
        axios.post(Environment.rootWeb + '/' + PEOPLE_PICKER_URL, query, httpOptions).then(results => {
          let jsonData = JSON.parse((results as any).data.d.ClientPeoplePickerSearchUser);
          resolve(jsonData);
        }).catch(() => {
          resolve([]);
        });
